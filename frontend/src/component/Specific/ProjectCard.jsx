...
import { useGetProjectSpecificTaskMutation, useGetSpecficProjectDetailsQuery, useGetSpecificProjectStatusCardQuery } from "../../redux/api/api";
...
const LazyAssignNewTaskDialog=lazy(()=>import('../Dialog/AssignTaskProjectDialog'));
...
const {data:ProjectStatusCard,isLoading:loadingProjectStatusCard}=useGetSpecificProjectStatusCardQuery(projectId);

console.log('udta punjaab',ProjectStatusCard)
...
const statusCards = [
  { id: 1, title: "Total Task", count: ProjectStatusCard?.totalTasks},
  { id: 2, title: "Assigned Task", count: ProjectStatusCard?.totalAssignedTask},
  { id: 3, title: "Incomplete Task", count: ProjectStatusCard?.incompleteTask },
  { id: 4, title: "Completed Task", count: ProjectStatusCard?.CompletedTask },
...
console.log("AssignedTo:", tasks);
  return (
    ...
    <Suspense fallback={<CircularProgress sx={{display:"flex",justifyContent:"center"}} />}>
      {assignTaskDialog && <LazyAssignNewTaskDialog open={assignTaskDialog} onClose={()=> setassignTaskDialog(false)}  projectId={projectId} projectName={ProjectFQuery?.name} />}
    </Suspense>

    {loadingProjectquery || loadingProjectStatusCard?( <LayoutLoader />):(
    <div style={{ padding: 20,width:"100%" }}>
      ...
      <Grid container spacing={2}>
        {statusCards.map((title) => (
          <Grid item xs={12} sm={6} md={3} key={title.id}>
            <Card variant="outlined">
              <CardContent >
                <Grid container spacing={1} alignItems="center">
                  <Grid item width={'100%'} alignContent={'center'}  textAlign={'center'}>
                    <CalendarMonthIcon fontSize="large" color="primary" />
                  </Grid>
                  <Grid item  textAlign={'center'} alignContent={'center'} width={'100%'}>
                    <Typography variant="subtitle2">{title.title}</Typography>
                    <Typography variant="h6" fontWeight="bold" >{title.count}</Typography>
      ...
      <TableContainer component={Paper} elevation={5} sx={{mt:1}}>
       ...
       <Grid container spacing={2} alignItems="center" sx={{ mt: 2 }}>
        ...
        <Grid item>
          <Button variant="contained" color="primary" onClick={()=>setassignTaskDialog(true)} >+ Assign Task</Button>
        </Grid>
      </Grid>
