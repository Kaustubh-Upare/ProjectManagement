...
const deleteProject=tryCatcher(async(req,res,next)=>{
  ...
  await Notification.create({
    ...
    reciever:pk.teamMembers,
    body:`${pk.name} project has been succesfully Deleted`
  })
  emitEvent(req,'Pro_Delete',pk.teamMembers,"Project has been Delete Succesfully")
...
const getAllTaskDetails=tryCatcher(async(req,res,next)=>{
  ...
  res.status(200).json({
    ...
    totalTasks,
    completedTasks,
    performance: ((completedTasks / totalTasks) * 100).toFixed(2),
  });
})
...
module.exports={AdminDashboardStats,adminUsersTaskData,getallProjects,
  ...
  changeTaskStatus,deleteTaskFiles,getAllChats,addTask,getAllTaskDetails
}