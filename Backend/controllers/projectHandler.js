...
const projectCardStats=tryCatcher(async(req,res,next)=>{
    const {id}=req.params;
    if(!id){
        return next(new ErrorHandler("Check THe Routes please",404));
    }
    const pd=await Project.findById(id).populate('tasks','assignedTo status');
    const userTasks = pd.tasks.filter(task =>
        task.assignedTo.some(assignee => assignee.toString() === req.usero)
      );
    
    const totalAssignedTask = userTasks.length;
    const CompletedTask=userTasks.filter(ta=>ta.status ==='Completed').length;
    const incompleteTask = userTasks.filter(task => task.status !== 'Completed').length;
    const totalTasks = pd.tasks.length;
...
module.exports={NewProject,updateProject,getMyProjects,getMyTeamMembers
    ,getProjectDetails,getProjectMembers,editProject,DeleteProject,projectCardStats};