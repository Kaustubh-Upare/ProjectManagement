...
const getMyNotifications=tryCatcher(async(req,res,next)=>{
    ...
    const allNotifications=notify.map(({_id,sender,body,createdAt,updatedAt})=>({
        _id,
        sender:{
            _id:sender._id,
            name:sender.name
...
const dashboardSpecficTasks=tryCatcher(async(req,res,next)=>{
    
    const now = new Date();
    const [TotalTasks,TotalTaskCompleted,
        TotalTaskInProgress,
        TotalTaskInReview,
        TotalOverdueTasks,]=await Promise.all([
            Task.countDocuments({assignedTo:req.usero}),
            Task.countDocuments({ assignedTo: req.usero, status: 'Completed' }),
            Task.countDocuments({ assignedTo: req.usero, status: 'In Progress' }),
            Task.countDocuments({ assignedTo: req.usero, status: 'In Review' }),
            Task.countDocuments({
                assignedTo: req.usero,
                deadline: { $lt: now },
                status: { $ne: 'Completed' }
              }),
        ])
